...
def confirm_adv_attack(target_classifier,
    ...
    try:
        ...
        max = np.max(l0s)
        avg = np.average(l0s)
        logger.debug(f'L0 min/max/avg = {min} / {max} / {avg} (close to 0 is better)')

        l2s = compute_l2s(final_advs, final_origin, n_features=final_advs.shape[1] * final_advs.shape[2] * final_advs.shape[3])
        min = np.min(l2s)
        max = np.max(l2s)
        avg = np.average(l2s)
        logger.debug(f'L2 min/max/avg = {min} / {max} / {avg} (close to 0 is better)')

        ssims = compute_ssim(final_advs, final_origin)
        min = np.min(ssims)
        max = np.max(ssims)
        avg = np.average(ssims)
        logger.debug(f'SSIM min/max/avg = {min} / {max} / {avg} (larger value is better)')
