...
def filter_by_label(label: int, data_set: np.ndarray, label_set: np.ndarray) -> (np.ndarray, np.ndarray):
    ...
    for data_i, label_i in zip(data_set, label_set):
        if (np.argmax(label_i) == label and len(label_i) > 1):
...
def filter_candidate_adv(cnn_model, origin_data: np.ndarray, candidate_adv: np.ndarray, target_label: int,
                         is_untargeted: bool = False, origin_label: int = None, num_class: int = None):
    ...
    if is_untargeted:
        ...
        for i in range(origin_data.shape[0]):
            origin_confident_i = np.argmax(origin_confident[i]) if num_class > 1 else np.round(origin_confident[i])
            candidate_confident_i = np.argmax(candidate_confident[i]) if num_class > 1 else np.round(
                candidate_confident[i])

            if origin_label == -1 and origin_confident_i != candidate_confident_i:
            ...
            elif origin_confident_i == origin_label and origin_confident_i != candidate_confident_i:
                result_adv_data.append(candidate_adv[i])
                result_adv_labels_vector.append(candidate_confident[i])
        ...
        for i in range(origin_data.shape[0]):
            ...
            if origin_confident_i != target_label and candidate_confident_i == target_label:
                result_adv_data.append(candidate_adv[i])
                result_adv_labels_vector.append(candidate_confident[i])
...
def plot_images(origin_image_worst, origin_image_best, gen_image_worst, gen_image_best, l2_worst, l2_best,
    ...
    origin_image_title = 'origin: {origin: d}, {probability: .0f}, index: {index: d}'.format(
        origin=np.argmax(origin_label),
